...
class User_model extends CI_Model
{
	...
	public function create($name,$email,$password,$accesslevel,$status,$socialid,$logintype,$image,$json,$dob,$interest)
	{
		$data  = array(
			'name' => $name,
			'email' => $email,
        ...
        {
            $this->user_model->createinterestbyuser($value,$id);
        }
		if(!$query)
			return  0;
		else
    ...
    public function createinterestbyuser($value,$user)
	{
		$data  = array(
			'interest' => $value,
			'user' => $user
		);
		$query=$this->db->insert( 'userinterest', $data );
		return  1;
	}
	...
	public function edit($id,$name,$email,$password,$accesslevel,$status,$socialid,$logintype,$image,$json,$dob,$interest)
	{
		$data  = array(
			'name' => $name,
			'email' => $email,
        ...
        $querydelete=$this->db->query("DELETE FROM `userinterest` WHERE `user`='$id'");
        foreach($interest AS $key=>$value)
        {
            $this->user_model->createinterestbyuser($value,$id);
        }
...
//        }
    ...
    public function signupfrontenduser($name,$email,$dob)
    {
        $insertquery=$this->db->query("INSERT INTO `user`(`name`,`email`,`dob`) VALUES ('$name','$email','$dob')");
    ...
    public function adduserinterest($userid,$interest)
	{
        
        foreach($interest AS $key=>$value)
        {
            $this->user_model->createinterestbyuser($value,$userid);
        }
		return  1;
	}
}
